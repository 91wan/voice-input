...
- `make version-bump` は最新の configured release branch（デフォルト `main`）で実行し、metadata mutation の前に local HEAD が `origin/main` と一致している必要があります。
- 別の remote/branch を使う場合は `make version-bump VERSION=vX.Y.Z REMOTE=upstream RELEASE_BRANCH=main` を実行します。
